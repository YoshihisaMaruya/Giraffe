    ...
    for(int i = 0; i < mats.size(); i++) {
        for(int j = 0; j < mats[i].rows; j++){
            for(int k = 0; k < mats[i].cols; k++){
                double elem = mats[i].at<double>(j,k);
                CV_MAT_ELEM(*objMat,double,count,k) = elem;
            }
            count++;
        }
    }
...
extern "C" {
    ...
    void init(){
         lsh = cvCreateMemoryLSH(128, 100000, 10, 64, CV_32FC1); //lshの作成(メモリに領域確保)
    }
    ...
    void readFromMstImg(char* dir_path,int count){
        ...
        for(int i = 0; i < count; i++){
            Mat training_image_mat = imread(CharsToString(dir_path) + "/" + IntToString(i) + ".jpg");
            //SURF特徴点検出器 
    ...
    int* match(int query_keypoints_size,int rows,int cols,float* query_descriptors){
        ...
        CvMat* queryMat = createCvMatDescriptors(rows,cols,query_descriptors);
