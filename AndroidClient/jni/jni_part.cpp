...
SurfFeatureDetector surf_detector(300);
...
JNIEXPORT void JNICALL Java_com_example_detectimage_CameraPreview_setTrainingImages(
		JNIEnv* env, jobject thiz, jintArray widths, jintArray heights,
		jobjectArray rgbas, jint imageNum) {
	matcher.clear(); //matcherの初期化
...
JNIEXPORT void JNICALL Java_com_example_detectimage_DetectImageActivity_setTrainingImage(
		JNIEnv* env, jobject thiz, jint width, jint height, jintArray rgba) {
	matcher.clear(); //matcherの初期化
	LOGV("setTrainingImages");
	...
	//各画像に対し、特徴量を抽出し特徴量照合器(matcher)へ登録
	jint* _rgba = env->GetIntArrayElements(rgba, 0);
	Mat mrgba(height, width, CV_8UC4, (unsigned char *) _rgba); //ピクセルデータをMatへ変換

	Mat gray(height, height, CV_8UC1);
	cvtColor(mrgba, gray, CV_RGBA2GRAY, 0); //グレースケールへ変換
